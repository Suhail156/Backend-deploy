...
export const login=async(req,res)=>{
  ...
  //  console.log(validpass);
   ...
   //jwt
   const token=Jwt.sign({id:uservalid._id},process.env.USER_SECRET_TOKEN,{expiresIn:"10m"})
  ...
  const refreshToken=Jwt.sign({id:uservalid._id},process.env.REFRESH_ACCESS_TOKEN)
   ...
   return res.status(200).json({ message: "successfully login", rest: rest,token:token,refreshToken:refreshToken });
}